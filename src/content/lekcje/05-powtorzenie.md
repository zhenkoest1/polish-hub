---
numer: 5
tytul: "Wielkie powtórzenie — lekcje 1–4"
emoji: "🔄"
gramatyka: "Powtórzenie: lekcje 1–4"
opis: "Cztery lekcje, cztery tematy gramatyczne, jeden wieczór. Stopniowanie, biernik, narzędnik, modalne i celownik — wszystko razem, w jednym miejscu."
quiz: "05-powtorzenie"
---

## 🗺️ Co powtarzamy

| Blok | Temat gramatyczny | Skąd | Pytania |
|---|---|---|---|
| 1 | Stopniowanie przymiotników | Lekcja 1 — Tatry | *wyższy · najwyższy* |
| 2 | Biernik | Lekcja 2 — Społeczeństwo zmęczenia | *kogo? co?* |
| 3 | Narzędnik | Lekcja 3 — Żelazne prawo oligarchii | *kim? czym?* |
| 4 | Czasowniki modalne | Lekcja 4 — Sztuka kompromisu | *musieć · móc · chcieć · powinien* |
| 5 | Celownik | Lekcja 4 — Sztuka kompromisu | *komu? czemu?* |

## 📐 Blok 1 — stopniowanie przymiotników

Polskie przymiotniki mają trzy stopnie:

- **Równy:** *wysoki, trudny, dobry*
- **Wyższy:** temat + **-szy / -iejszy** → *wyższy, trudniejszy*
- **Najwyższy:** **naj-** + stopień wyższy → *najwyższy, najtrudniejszy*

**Formy nieregularne — na pamięć:**

| Równy | Wyższy | Najwyższy |
|---|---|---|
| duży | **większy** | **największy** |
| mały | **mniejszy** | **najmniejszy** |
| dobry | **lepszy** | **najlepszy** |
| zły | **gorszy** | **najgorszy** |
| dużo | **więcej** | **najwięcej** |

Porównanie: **A jest -szy NIŻ B.** → *Rysy są wyższe **niż** Giewont.*

> 📈 [Pełna reguła: Stopniowanie przymiotników →](/polish-hub/reguly/stopniowanie/)

## 📐 Blok 2 — biernik *(kogo? co?)*

Po czasownikach akcji: *mieć, widzieć, czuć, pisać, czytać, kupić, robić, sprawdzać, planować, znać…*

| Rodzaj | Mianownik → Biernik | Przykład |
|---|---|---|
| Męski nieżywotny | **= mianownik** | telefon → **telefon** |
| Męski żywotny | **= dopełniacz (-a)** | wróg → **wroga** |
| Żeński | **-a → -ę** | firma → **firmę** |
| Nijaki | **= mianownik** | zmęczenie → **zmęczenie** |

Przymiotniki: męski żywotny **-ego** + rzeczownik **-a** *(surowego szefa)*; żeński **-ą** + **-ę** *(nową aplikację)*; męski nieżywotny i nijaki = mianownik.

> ⚠️ **Po NIE biernik zmienia się w dopełniacz:** *Mam sukces → Nie mam **sukcesu**.*

> 🎯 [Pełna reguła: Biernik →](/polish-hub/reguly/biernik/)

## 📐 Blok 3 — narzędnik *(kim? czym?)*

Po **być** *(kim/czym)*, **stać się** *(kim)*, **zarządzać** *(czym)*, po przyimkach **z**, **między**, **nad**, **pod**.

| Rodzaj | Mianownik → Narzędnik | Przykład |
|---|---|---|
| Męski | **-em / -iem** | socjolog → socjolog**iem** |
| Męski na -a | **-a → -ą** | socjalista → socjalist**ą** |
| Żeński | **-a → -ą** | organizacja → organizacj**ą** |
| Nijaki | **-em** | prawo → praw**em** |
| Liczba mnoga | **-ami** | liderzy → lider**ami** |

Przymiotniki: męski i nijaki **-ym / -im**; żeński **-ą**; liczba mnoga **-ymi / -imi**.

> 🛠️ [Pełna reguła: Narzędnik →](/polish-hub/reguly/narzednik/)

## 📐 Blok 4 — czasowniki modalne

| | musieć | móc | chcieć |
|---|---|---|---|
| ja | **muszę** | **mogę** | **chcę** |
| ty | **musisz** | **możesz** | **chcesz** |
| on / ona | **musi** | **może** | **chce** |
| my | **musimy** | **możemy** | **chcemy** |
| wy | **musicie** | **możecie** | **chcecie** |
| oni / one | **muszą** | **mogą** | **chcą** |

**POWINIEN** zmienia się przez rodzaje: *powinienem* (m.), *powinnam* (ż.), *powinien / powinna* (on / ona), *powinni / powinny* (oni / one).

> ⚠️ **musieć** = jestem zmuszony *(nie ma wyboru)* · **powinien** = tak byłoby dobrze *(wybór jest)*.

**Bezosobowe:** *trzeba* (konieczność) · *można* (możliwość) · *wolno* (pozwolenie) · *należy* (formalnie).

> 💪 [Pełna reguła: Czasowniki modalne →](/polish-hub/reguly/modalne/)

## 📐 Blok 5 — celownik *(komu? czemu?)*

Po czasownikach: *pomagać, ufać, wierzyć, wybaczać, dziękować, mówić, dawać, przeszkadzać, ustępować, służyć…*

| Rodzaj | Mianownik → Celownik | Przykład |
|---|---|---|
| Męski | **-owi** *(wyjątki na -u)* | przyjaciel → przyjaciel**owi**, brat → brat**u** |
| Męski na -a | jak żeński | kolega → **koledze** |
| Żeński | **-e** *(z wymianą)* albo **-i / -ii** | mama → mam**ie**, pani → pan**i** |
| Nijaki | **-u** | dziecko → dzieck**u** |

**Zaimki:** ja → **mi** · ty → **ci** · on → **mu** · ona → **jej** · my → **nam** · wy → **wam** · oni/one → **im**

**Bezosobowe:** *wolno mi · przykro mi · smutno mi · głupio mi*

> 🤲 [Pełna reguła: Celownik →](/polish-hub/reguly/celownik/)

## 🗣️ Na spotkaniu — ćwiczenia ustne

*(ćwiczenia pisemne robicie od razu na stronie — w polach niżej; na koniec czeka jeszcze quiz)*

### Runda 1 — stopniowanie

Piętnaście zdań, każdy wpisuje sam. Przed każdym kliknięciem sprawdź dwie rzeczy naraz: **jaki to rodzaj** i **czy w zdaniu stoi „niż”** — bo bez „niż” potrzebny jest stopień najwyższy.

```cwiczenie
{ "typ": "wpisz", "tytul": "Wyższy czy najwyższy?", "instrukcja": "Wpisz słowo z nawiasu we właściwym stopniu i rodzaju. Przymiotniki w formie prostej (na -szy / -iejszy), nie opisowej z „bardziej”.",
  "zdania": [
    { "przed": "(słynny) Morskie Oko jest ", "odpowiedz": ["słynniejsze"], "po": " niż Wielki Staw Polski.", "czemu": "Morskie Oko to rodzaj nijaki → -e; słynny → słynniejszy (-iejszy)" },
    { "przed": "(ciężki) Niedźwiedź jest ", "odpowiedz": ["cięższy"], "po": " niż kozica.", "czemu": "niedźwiedź: rodzaj męski; ciężki → cięższy (jak szybki → szybszy)" },
    { "przed": "(wysoki) Rysy to ", "odpowiedz": ["najwyższy"], "po": " szczyt w polskich Tatrach.", "czemu": "nie ma «niż» → stopień najwyższy: naj- + wyższy; określamy słowo szczyt, czyli rodzaj męski" },
    { "przed": "(poważny) Wypalenie jest ", "odpowiedz": ["poważniejsze"], "po": " niż zwykłe zmęczenie.", "czemu": "wypalenie: rodzaj nijaki → -e; poważny → poważniejszy" },
    { "przed": "(dobry) Demokracja jest ", "odpowiedz": ["lepsza"], "po": " niż oligarchia.", "czemu": "nieregularne: dobry → lepszy; demokracja jest żeńska → lepsza" },
    { "przed": "(łatwy) Kolejka linowa jest ", "odpowiedz": ["łatwiejsza"], "po": " niż wędrówka pieszo.", "czemu": "kolejka: rodzaj żeński → -a; łatwy → łatwiejszy" },
    { "przed": "(trudny) Kontemplacja jest ", "odpowiedz": ["trudniejsza"], "po": " niż scrollowanie telefonu.", "czemu": "kontemplacja: rodzaj żeński → -a; trudny → trudniejszy" },
    { "przed": "(mały) Świstak to ", "odpowiedz": ["najmniejsze"], "po": " zwierzę z trzech: niedźwiedź, kozica, świstak.", "czemu": "«z trzech» → stopień najwyższy; zwierzę jest nijakie; mały → mniejszy → najmniejszy (nieregularne)" },
    { "przed": "(niski) Giewont jest ", "odpowiedz": ["niższy"], "po": " niż Rysy.", "czemu": "Giewont: rodzaj męski; niski → niższy" },
    { "przed": "(dobry) Kompromis jest ", "odpowiedz": ["lepszy"], "po": " niż konflikt.", "czemu": "kompromis: rodzaj męski; nieregularne dobry → lepszy" },
    { "przed": "(głęboki) Wielki Staw Polski jest ", "odpowiedz": ["głębszy"], "po": " niż Morskie Oko.", "czemu": "staw: rodzaj męski; głęboki → głębszy" },
    { "przed": "(dużo) Lider wie ", "odpowiedz": ["więcej"], "po": " niż zwykły członek organizacji.", "czemu": "nieregularne: dużo → więcej → najwięcej; to forma przysłówkowa, nie zmienia się przez rodzaje" },
    { "przed": "(nowy) Społeczeństwo osiągnięć jest ", "odpowiedz": ["nowsze"], "po": " niż społeczeństwo dyscypliny.", "czemu": "społeczeństwo: rodzaj nijaki → -e; nowy → nowszy" },
    { "przed": "(zły) Samoeksploatacja jest ", "odpowiedz": ["gorsza"], "po": " niż eksploatacja przez szefa.", "czemu": "nieregularne: zły → gorszy; samoeksploatacja jest żeńska → gorsza" },
    { "przed": "(popularny) Tatrzański Park Narodowy to ", "odpowiedz": ["najpopularniejszy"], "po": " park w Polsce.", "czemu": "nie ma «niż» → stopień najwyższy; park jest męski; popularny → popularniejszy → najpopularniejszy" }
  ] }
```

Po sprawdzeniu przeczytajcie zdania na głos, po kolei — każdy swoją piątkę.

### Runda 2 — tłumaczenie na głos

**Osoba A:** 1. Гевонт ниже, чем Рысы. 2. Морское Око — самое известное озеро в Татрах. 3. Выгорание — худшая болезнь нашего времени. 4. Демократия лучше, чем олигархия.

**Osoba B:** 1. Сурок меньше, чем серна. 2. Медведь — самое большое животное в Татрах. 3. Компромисс труднее, чем упрямство. 4. Канатная дорога — самый лёгкий способ подняться на вершину.

**Osoba C:** 1. Великий Став Польский глубже, чем Морское Око. 2. Самоэксплуатация — самый опасный вид усталости. 3. Юм был более гибким, чем Кант. 4. Либерум вето было самым плохим правилом в польском сейме.

### Runda 3 — narzędnik

Wpisz samą odpowiedź — słowo z nawiasu w narzędniku, razem z przymiotnikiem, jeśli jakiś tam stoi. Pamiętaj o pułapce: po **być / zostać / stać się** zawód i rola idą w narzędniku, ale po *to jest* zostaje mianownik.

```cwiczenie
{ "typ": "wpisz", "tytul": "Kim? Czym?", "instrukcja": "Wpisz wyrażenie z nawiasu w narzędniku.",
  "zdania": [
    { "przed": "Kim był Arystoteles? (grecki filozof) — Arystoteles był ", "odpowiedz": ["greckim filozofem"], "po": ".", "czemu": "po «być» rola idzie w narzędniku; filozof → filozofem (-em), grecki → greckim (-im po k/g)" },
    { "przed": "Czym jest samoeksploatacja? (więzienie bez krat) — Samoeksploatacja jest ", "odpowiedz": ["więzieniem bez krat"], "po": ".", "czemu": "nijakie więzienie → więzieniem (-em); dokładnie tak nazywa to Han" },
    { "przed": "Z kim rozmawiają liderzy? (inni liderzy) — Liderzy rozmawiają z ", "odpowiedz": ["innymi liderami"], "po": ".", "czemu": "po przyimku «z» narzędnik; liczba mnoga: liderzy → liderami (-ami), przymiotnik → innymi (-ymi)" },
    { "przed": "Kim staje się rewolucjonista? (konserwatysta) — Rewolucjonista staje się ", "odpowiedz": ["konserwatystą"], "dokladnie": true, "po": ".", "czemu": "rzeczownik męski na -a odmienia się jak żeński: -a → -ą (jak socjalista → socjalistą) — końcówka -ą, nie -a" },
    { "przed": "Kim był Kant? (niemiecki filozof) — Kant był ", "odpowiedz": ["niemieckim filozofem"], "po": ".", "czemu": "niemiecki → niemieckim (-im po k/g), filozof → filozofem" },
    { "przed": "Czym jest organizacja według Michelsa? (narzędzie) — Organizacja jest ", "odpowiedz": ["narzędziem"], "po": ".", "czemu": "narzędzie → narzędziem (rodzaj nijaki, -e → -em)" },
    { "przed": "Z kim rozmawiali komuniści przy Okrągłym Stole? (opozycja) — Komuniści rozmawiali z ", "odpowiedz": ["opozycją"], "dokladnie": true, "po": ".", "czemu": "przyimek «z» + narzędnik; żeński -a → -ą — końcówka -ą, nie -a" },
    { "przed": "Kim jest Byung-Chul Han? (profesor w Berlinie) — Han jest ", "odpowiedz": ["profesorem w Berlinie"], "po": ".", "czemu": "profesor → profesorem (-em); zmienia się tylko rola, nie miejsce" },
    { "przed": "Czym jest liberum veto? (absolutna sztywność) — Liberum veto jest ", "odpowiedz": ["absolutną sztywnością"], "po": ".", "czemu": "żeński na spółgłoskę + -ą: sztywność → sztywnością; przymiotnik żeński → absolutną" },
    { "przed": "Kim był Michels? (socjolog i członek SPD) — Michels był ", "odpowiedz": ["socjologiem i członkiem SPD"], "po": ".", "czemu": "po k/g końcówka -iem: socjolog → socjologiem, członek → członkiem (uwaga: «e» wypada)" },
    { "przed": "Czym zarządzają liderzy? (organizacja) — Liderzy zarządzają ", "odpowiedz": ["organizacją"], "dokladnie": true, "po": ".", "czemu": "czasownik zarządzać sam wymaga narzędnika, bez przyimka; żeński -a → -ą — końcówka -ą, nie -a" },
    { "przed": "Między kim leży kompromis? (dwie strony) — Kompromis leży między ", "odpowiedz": ["dwiema stronami", "dwoma stronami"], "po": ".", "czemu": "przyimek «między» + narzędnik; liczba mnoga -ami, liczebnik → dwiema (jak: wybór między dwiema drogami). Forma «dwoma stronami» też jest poprawna — słownik PWN dopuszcza obie." }
  ] }
```

Po sprawdzeniu powtórzcie każdą odpowiedź na głos, pełnym zdaniem.

### Runda 4 — składanie zdań: modalny + czasownik + celownik

Tu poprawnych wersji jest wiele — osoba, czas i kontekst zależą od ciebie. Ułóż każde zdanie sam i zapisz je; na spotkaniu przeczytacie zdania na głos i grupa poprawi.

```cwiczenie
{ "typ": "pisanie", "tytul": "Modalny + czasownik + celownik", "instrukcja": "Ułóż z każdego zestawu jedno pełne zdanie. Modalny w formie osobowej, drugi czasownik w bezokoliczniku, osoba na końcu — w celowniku.",
  "zdania": [
    { "przed": "musieć + pomóc + siostra", "po": "", "czemu": "pomagać komuś: siostra → siostrze (żeński -e z wymianą, jak mama → mamie)" },
    { "przed": "powinien + wybaczyć + kolega", "po": "", "czemu": "powinien zmienia się przez rodzaje: powinienem / powinnam; kolega → koledze (męski na -a odmienia się jak żeński)" },
    { "przed": "chcieć + zaufać + Oleg", "po": "", "czemu": "ufać komuś; imię męskie → Olegowi (standardowa końcówka -owi)" },
    { "przed": "trzeba + pomóc + mama", "po": "", "czemu": "«trzeba» nie ma podmiotu i bierze sam bezokolicznik; mama → mamie" },
    { "przed": "nie móc + ustąpić + on", "po": "", "czemu": "ustępować komuś; zaimek on → mu w środku zdania, jemu z naciskiem" },
    { "przed": "nie wolno + kłamać + przyjaciel", "po": "", "czemu": "«nie wolno» + bezokolicznik — ulubiona konstrukcja Kanta; komu? — celownik: przyjaciel → przyjacielowi (standardowa końcówka -owi, jak w tabeli wyżej)" }
  ] }
```

### 💬 Dyskusja — połączcie cztery lekcje

Wybierzcie 3 pytania. Każdy musi użyć minimum jednego przykładu z każdej lekcji:

1. **Czy Han i Michels mówią o tym samym?** Samoeksploatacja i żelazne prawo oligarchii — czy oba to pułapki bez wroga?
2. **Co powiedziałby Kant o „liberum veto”?** A Hume?
3. **Czy odpoczynek według Hana to kompromis z samym sobą?**
4. **Który tekst z lekcji 1–4 był dla was najtrudniejszy — i dlaczego?** *(użyjcie stopnia najwyższego!)*
5. **Gdyby Michels usiadł przy Okrągłym Stole — co by powiedział?**

**Przydatne konstrukcje:** *Moim zdaniem… / Wydaje mi się, że… / Zgadzam się z…, bo… / Trzeba przyznać, że… / Nie mam pewności, ale…*

---

*Lekcja powtórzeniowa. Materiał z lekcji 1–4.*
